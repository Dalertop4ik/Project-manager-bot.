...
/info - выводит информацию о командах бота
...
def handle_photo(message):
    """Обрабатывает получение фото проекта от пользователя."""
    file_info = bot.get_file(message.photo[-1].file_id)
    downloaded_file = bot.download_file(file_info.file_path)

    with open(f'projects/{file_info.file_path}', 'wb') as new_file:
...
def set_description_command(message):
    """Обрабатывает команду для установки описания проекта."""
...
def save_description(message):
    """Сохраняет описание проекта в БД."""
    description = message.text
    user_id = message.from_user.id
    # Логика сохранения описания в БД
